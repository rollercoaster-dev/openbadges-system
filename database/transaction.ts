...
export async function withSavepoint<T>(
  ...
  try {
    ...
    await trx.executeQuery(
      trx.schema.raw(`SAVEPOINT ${savepointName}`).compile()
    );
    ...
    await trx.executeQuery(
      trx.schema.raw(`RELEASE SAVEPOINT ${savepointName}`).compile()
    );
    ...
    await trx.executeQuery(
      trx.schema.raw(`ROLLBACK TO SAVEPOINT ${savepointName}`).compile()
    );
    throw error;
  }
}
