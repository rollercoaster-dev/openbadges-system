...
async function run() {
  try {
    ...
    const status = complianceIssues.length === 0 ? 'pass' : 
                  complianceIssues.some(i => i.severity === 'error') ? 'fail' : 'warning';
    ...
    if (complianceIssues.length > 0) {
      ...
      if (status === 'fail') {
        core.setFailed(`Found ${complianceIssues.filter(i => i.severity === 'error').length} compliance error(s)`);
      }
    } else {
          ...
          if (content.includes('verifyBadge') || content.includes('validateBadge')) {
            if (!content.includes('signature') && !content.includes('proof')) {
              return {
                ...
                suggestion: 'Implement signature or proof verification according to OpenBadges spec'
              };
            }
          }
          return null;
        ...
        check: (content) => {
          if (content.includes('@context') && content.includes('openbadges')) {
            try {
              ...
              if (json['@context'] && !json['@context'].includes('https://w3id.org/openbadges/v2')) {
                return {
                  severity: 'warning',
                  message: 'OpenBadges context URL may be outdated',
                  suggestion: 'Use https://w3id.org/openbadges/v2 for OpenBadges 2.x or appropriate 3.0 context'
                };
              }
            } catch (e) {
              // Not valid JSON, skip
        ...
        check: (content) => {
          ...
          if (content.includes('md5') || content.includes('sha1')) {
            return {
              ...
              suggestion: 'Use SHA-256 or stronger algorithms for OpenBadges cryptographic operations'
            };
          }
          
          if (content.includes('private') && content.includes('key') && !content.includes('process.env')) {
            return {
              severity: 'error',
